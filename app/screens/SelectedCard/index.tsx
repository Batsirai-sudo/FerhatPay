...
import { WalletGradient } from '@config';
...
const WalletTransactions = () => {
	...
	useEffect(() => {
		...
		setOptions({
			headerStyle: {
				backgroundColor: WalletGradient.gradientsFor('first'),
				borderBottomWidth: 0,
				elevation: 0,
				// shadowRadius: 0,
		...
		return (
           ...
           */}
				...
				<View style={[styles.listHeaderTextRow, stylesHook.listHeaderTextRow]}>
					<Text style={[styles.listHeaderText, stylesHook.listHeaderText]}>{'Transactions'}</Text>
					<TouchableOpacity
		...
		return (
			<BottomModal isVisible={isManageFundsModalVisible} onClose={hideManageFundsModal}>
				<KeyboardAvoidingView enabled={!Platform.isPad} behavior={Platform.OS === 'ios' ? 'position' : null}>
					<View
						style={[
							styles.advancedTransactionOptionsModalContent,
							stylesHook.advancedTransactionOptionsModalContent,
						]}
					>
						<BlueListItem
							hideChevron
						...
						{/* <BlueListItem
							...
							onPress={() => {
								setIsManageFundsModalVisible(false);
								Linking.openURL(
									'https://zigzag.io/?utm_source=integration&utm_medium=bluewallet&utm_campaign=withdrawLink'
								);
							}}
						/> */}
					</View>
	...
	const renderMarketplaceButton = () => {
		return Platform.select({
			android: (
				...
				>
					<Text style={[styles.marketpalceText1, stylesHook.marketpalceText1]}>
						{loc.wallets.list_marketplace}
					</Text>
				</TouchableOpacity>
			),
			ios:
				wallet.getBalance() > 0 ? (
					...
					>
						...
						<Text style={[styles.marketpalceText2, stylesHook.marketpalceText2]}>
							{loc.wallets.list_marketplace}
						</Text>
					</TouchableOpacity>
				) : null,
	...
	const renderLappBrowserButton = () => {
		return (
			...
			>
				<Text style={[styles.marketpalceText1, stylesHook.marketpalceText1]}>{'Account Statement'}</Text>
			</TouchableOpacity>
		);
	};
	...
	const renderItem = (item) => (
		<BlueTransactionListItem item={item.item} itemPriceUnit={'itemPriceUnit'} timeElapsed={timeElapsed} />
	);
	...
	return (
        ...
        />
			...
			<Header
				containerStyle={{ backgroundColor: WalletGradient.gradientsFor(type)[0] }}
				rightComponent={() => (
					<TouchableOpacity
				...
				leftComponent={() => (
					<TouchableOpacity
						testID="WalletDetails"
						// disabled={route.params.isLoading === true}
						style={{ width: 50, left: -15 }}
						onPress={() => goBack()}
					>
						<Icon name="chevron-left" type="octicon" size={30} color="#FFFFFF" />
					</TouchableOpacity>
				)}
				// title: '',
				// headerStyle: {
			...
			<View style={{ marginHorizontal: 20 }}>
				<FContainer ref={walletActionButtonsRef}>
					...
					{/* {(wallet.allowSend() || (wallet.type === .typeWatchOnlyWallet && wallet.isHd() && wallet.getSecret().startsWith('zpub'))) && ( */}
					{true && (
						<FButton
