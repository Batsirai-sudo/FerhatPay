...
const SendDetails = () => {
	...
	//     return (
	//       <BottomModal deviceWidth={width + width / 2} isVisible={optionsVisible}
	//     //   onClose={hideOptions}
	//       >
	//         <KeyboardAvoidingView enabled={!Platform.isPad} behavior={Platform.OS === 'ios' ? 'position' : null}>
	...
	//             {true && (
	//               <BlueListItem title={'loc.send.details_adv_import'}
	//               hideChevron component={TouchableOpacity}
	//             //    onPress={importTransaction}
	//                 />
	//             )}
	//             {/* {wallet.type === WatchOnlyWallet.type && wallet.isHd() && wallet.getSecret().startsWith('zpub') && ( */}
	//             {true && (
	...
	//             <BlueListItem
	//             testID="CoinControl"
	//             title={'loc.cc.header'} hideChevron
	//             component={TouchableOpacity}
	...
	const renderBitcoinTransactionInfoFields = (params) => {
		...
		return (
			<View style={{ width }} testID={'Transaction' + index}>
				...
				/>
				{addresses.length > 1 && (
					<Text style={[styles.of, stylesHook.of]}>
	...
	const balance = 0;
	//    utxo ? utxo.reduce((prev, curr) => prev + curr.value, 0) : wallet.getBalance();
	//   const allBalance = formatBalanceWithoutSuffix(balance, BitcoinUnit.BTC, true);
	const allBalance = 452;
						...
						<View style={styles.avatarWrapper}>
							<View>
								<Image
									...
									source={
										require('./hodlhodl-default-avatar.png')
							...
							<View style={styles.paddingLeft10}>
								<View style={styles.flexRow}>
									...
									<Text style={styles.nicknameText}>{'Batsirai Muchareva'}</Text>
								</View>
	...
	return (
		<View style={{ height: '100%' }}>
			<Header
				rightComponent={() => (
					<TouchableOpacity
						...
						// }
					>
						<Icon name="cross" type="entypo" size={22} color="#FFFFFF" />
					</TouchableOpacity>
				)}
			/>
			...
			<View style={{ position: 'absolute', bottom: 10, width: '100%' }}>{renderCreateButton()}</View>
		</View>
	);
};
...
const styles = StyleSheet.create({
	...
	avatarWrapperWrapper: { backgroundColor: 'transparent', paddingTop: 16, paddingBottom: 16, width: 400 },
	avatarWrapper: { backgroundColor: 'transparent', flexDirection: 'row' },
	...
	traderRatingText2: { color: '#9AA0AA', left: 25 },
	itemText: { color: '#9AA0AA', paddingTop: 10 },
	itemPriceWrapperWrapper: {
		flexDirection: 'row',
		paddingTop: 10,
		paddingBottom: 10,
		alignItems: 'center',
	...
	itemSeparator: { height: 0.5, width: '100%', backgroundColor: 'rgba(255,255,255,.1)' },
