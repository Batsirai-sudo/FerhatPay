...
import { Biometric, Hepatics } from '@config';
...
const UnlockWith = () => {
	...
	const renderUnlockOptions = () => {
		if (isAuthenticating) {
			...
			if (
				(biometricType === Biometric.TouchID || biometricType === Biometric.Biometrics) &&
				...
				return (
					<TouchableOpacity disabled={isAuthenticating} onPress={unlockWithBiometrics}>
						<Image
							source={
								colorScheme === 'dark' ? require('./faceid-default.png') : require('./faceid-dark.png')
							}
							style={styles.icon}
						/>
					</TouchableOpacity>
