...
import { Hepatics, DeviceInformation, BlueClipboard } from '@config';
...
const Home = () => {
	...
	// { accountType: accounts[0].toUpperCase(), type: accounts[0], balance: 1 },
	// { accountType: accounts[1].toUpperCase(), type: accounts[1], balance: 2 },
	// { accountType: accounts[2].toUpperCase(), type: accounts[2], balance: 3 },
	// '',
	const [itemWidth, setItemWidth] = useState(width * 0.82 > 375 ? 375 : width * 0.82);
	const [isLargeScreen, setIsLargeScreen] = useState(
		Platform.OS === 'android'
			? DeviceInformation.DeviceType
			: width >= Dimensions.get('screen').width / 2 && DeviceInformation.DeviceType
	);
	...
	const [accountData, setAccountData] = useState(
		accounts.map((x) => {
			if (x === '') return '';
			return { accountType: x.toUpperCase(), type: x, balance: count++ };
		})
	);
	useFocusEffect(
		useCallback(() => {
	...
	// 		I18nManager.isRTL && Platform.OS !== 'android' ? [false].concat(allWallets) : allWallets.concat(false);
	...
	const refreshTransactions = (showLoadingIndicator = true, showUpdateStatusIndicator = false) => {
		...
		// refreshAllWalletTransactions(showLoadingIndicator, showUpdateStatusIndicator).finally(() =>
		// 	setIsLoading(false)
		// );
	};
	...
	const renderWalletsCarousel = () => {
		return (
			<WalletsCarousel
				...
				onPress={(index: any) => {
					console.log(index);
					navigate('SelectedCard', { type: accountData[index].type });
				}}
				// handleLongPress={handleLongPress}
				handleLongPress={() => {
					// console.log(Hepatics());
				}}
		...
		switch (section.section.key) {
			case WalletsListSections.CAROUSEL:
				return isLargeScreen ? null : (
					<BlueHeaderDefaultMain
						leftText={'Account Wallet'}
						// onNewWalletPress={
						// 	!carouselData.some((wallet) => wallet.type === PlaceholderWallet.type)
						// 		? () => navigate('AddWalletRoot')
						// 		: null
						// }
					/>
			...
			<View style={[styles.walletsListWrapper, stylesHook.walletsListWrapper]}>
				<SectionList
					...
					sections={[
						...
						{
							key: WalletsListSections.TRANSACTIONS,
							data: transactions.map((x) => {
								return { name: x, type: x, amount: 20000, recepient: 'Batsirai' };
							}),
						},
					]}
				/>
